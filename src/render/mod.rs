...
mod render_impl {
    ...
    use crate::render::constants::{REFLECTION_RECURSION_DEPTH, SPHERES_MAX_DIST, SPHERES_SHD};
    ...
    fn refract(i: Vec3, n: Vec3, eta_t: f64, eta_i: f64) -> Vec3 {
        ...
        if cos_i < 0.0 {
            return refract(i, -n, eta_t, eta_t);
        }
    ...
    fn spheres_intercects(
            ...
            if d > 0.0 && pt.x.abs() < 10.0 && pt.z > -30.0 && d < spheres_dist {
                ...
                *n = Vec3::new(0.0, 1.0, 0.0);
                material.diffuse_color =
                    if (((0.5 * hit_point.x + 1000.0) as i64) + (0.5 * hit_point.z) as i64) & 1_i64
                        == 1
                    {
                        ONE_VEC
                    } else {
        ...
        (
            spheres_dist.min(checkerboard_dist) < SPHERES_MAX_DIST,
            material,
        )
    }
    ...
    pub(crate) fn cast_ray(
        ...
        for light in lights {
            ...
            let (intersects, ..) = spheres_intercects(
                shadow_orig,
                light_dir,
                &mut shadow_point,
                &mut shadow_n,
...
pub fn render_spheres(framebuffer: &mut ImageMatrix, spheres: &Vec<Sphere>, lights: &Vec<Light>) {
    ...
    for j in 0..h {
        for i in 0..w {
            ...
            framebuffer.put_pixel(
                i,
                j,
                render_impl::cast_ray(ZERO_VEC, dir, spheres, lights, 0),
            )
        }
    }
}
